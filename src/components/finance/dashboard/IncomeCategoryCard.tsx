...
function groupByCategory(transactions: Transaction[]): CategoryData[] {
  const incomes = transactions.filter(
    (tx) => tx.kind === "income" && !tx.pending && !tx.archivedAt && tx.amount > 0,
  );
...
export function IncomeCategoryCard({
  ...
  if (data.length === 0) {
    return (
      <Card className="bg-card p-5 shadow-none sm:p-6">
        <h3 className="text-base font-semibold">Receitas por categoria</h3>
  ...
  return (
    <Card className="bg-card p-5 shadow-none sm:p-6">
      ...
      <div className="mt-4 flex flex-col items-center gap-4 sm:flex-row">
        <div className="size-32 shrink-0" role="img" aria-label="Gráfico de receitas por categoria">
