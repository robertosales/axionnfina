...
export class CategorizationEngine {
  ...
  private async loadMerchantRules(): Promise<void> {
    this.merchantRules = [
      {
        pattern: /uber|99pop|lyft/i,
        category_id: await this.getCategoryId("expense_transport"),
        priority: 10,
      },
      {
        pattern: /ifood|rappi|uber\s*eats|zé\s*delivery/i,
        category_id: await this.getCategoryId("expense_food"),
        priority: 10,
      },
      {
        pattern: /netflix|spotify|youtube\s*premium|disney\+|amazon\s*prime/i,
        category_id: await this.getCategoryId("expense_subscriptions"),
        priority: 10,
      },
      {
        pattern: /shell|ipiranga|petrobras|ale|br\sposto/i,
        category_id: await this.getCategoryId("expense_transport"),
        priority: 8,
      },
      {
        pattern: /mercado\s*livre|amazon|magazine\s*luiza|americanas|shopee/i,
        category_id: await this.getCategoryId("expense_shopping"),
        priority: 8,
      },
      {
        pattern: /farm[aá]cia|drogaria|droga\s*raia|pacheco/i,
        category_id: await this.getCategoryId("expense_health"),
        priority: 8,
      },
      {
        pattern:
          /supermercado|mercado|atacad[aã]o|extra|p[ãa]o\s*de\s*a[cç]uc[aá]r|carrefour|walmart/i,
        category_id: await this.getCategoryId("expense_food"),
        priority: 8,
      },
      {
        pattern: /uber|99|taxi|cabify/i,
        category_id: await this.getCategoryId("expense_transport"),
        priority: 8,
      },
      {
        pattern: /google|apple|microsoft|aws|azure|digitalocean/i,
        category_id: await this.getCategoryId("expense_subscriptions"),
        priority: 8,
      },
      {
        pattern: /pix\s*(enviado|recebido)|ted|doc|transfer[eê]ncia/i,
        category_id: await this.getCategoryId("transfer_internal"),
        priority: 10,
      },
      {
        pattern: /dividendo|jcp|rendimento|provento/i,
        category_id: await this.getCategoryId("investment_dividend"),
        priority: 10,
      },
      {
        pattern: /corretagem|taxa\s*cust[oó]dia|emolumentos/i,
        category_id: await this.getCategoryId("investment_fee"),
        priority: 10,
      },
  ...
  async categorize(transaction: Transaction, userId: string): Promise<CategorizationResult> {
    const description =
      `${transaction.merchant_name || ""} ${transaction.description}`.toLowerCase();
  ...
  private categorizeByMerchantRules(description: string): CategorizationResult {
    ...
    return {
      ...
      confidence: 0,
      source: "rule",
      reason: "No merchant rule matched",
    };
  }
  ...
  private async categorizeByUserRules(
    ...
    if (!rules) {
      ...
      const loadedRules: UserRule[] = (data || []).map((r) => ({
        id: r.id,
      ...
      rules = loadedRules;
      this.userRulesCache.set(userId, loadedRules);
    }
