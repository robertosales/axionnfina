...
DECLARE
  ...
  v_status text;
BEGIN
  SELECT invoice.status, cc.account_id
  ...
  IF v_status = 'confirmed' THEN RETURN 0; END IF;
  IF v_status <> 'review' THEN RAISE EXCEPTION 'Invoice is not awaiting confirmation'; END IF;

  FOR v_item IN
    SELECT * FROM public.credit_card_invoice_items
  ...
  UPDATE public.credit_card_invoices SET status = 'confirmed' WHERE id = p_invoice_id AND status = 'review';
  RETURN v_count;
END;
$$;
